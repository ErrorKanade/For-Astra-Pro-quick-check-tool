...
Python: 3.10+ (需与 SDK 位数一致，建议 64-bit)

依赖库: pip install numpy opencv-python openni

🚀 快速开始

1. SDK 准备由于本程序调用了底层 C++ 库，需要准备好 Orbbec 官方的 OpenNI 2.3 SDK。
   
2.确保以下文件夹结构存在：PlaintextRedist/
...
3. 配置路径在 test.py 中，根据实际存放位置修改 REDIST_PATH：PythonREDIST_PATH = r"C:\Your\Path\To\Win64-Release\tools\Redist"
   
4. 运行程序Bashpython test.py
 
🖥️ 窗口说明程序运行后将开启一个四分屏窗口，0
布局如下：
左上 (RGB) 右上 (Depth)调用 UVC 协议采集的彩色图像经过 BONE 伪彩色处理的深度图
左下 (IR) 右下 (Simulated PC)实时红外散斑图基于深度梯度的伪彩色模拟点云

⚠️ 常见问题排查 (Troubleshooting)无法检测到 RGB 相机？
Astra Pro 的彩色摄像头通常占用系统视频索引 0, 1 或 2。

若未显示，请尝试修改代码中的：cap_rgb = cv2.VideoCapture(2, cv2.CAP_DSHOW)。

出现 Access Violation 报错？
这是因为 OpenNI 找不到驱动。请确保 os.chdir(REDIST_PATH) 指向的文件夹内包含 OpenNI2/Drivers 目录。

画面不对齐？
Astra Pro 的 RGB 和 IR 镜头存在物理间距（视差）。
